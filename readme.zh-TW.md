...
- [system-one-adapter-python](https://github.com/typesafe-ai/system-one-adapter-python) - LLM 後端的 drop-in client，方便 A/B。
...
- [jev-ultrafast](https://github.com/browser-use/jev-ultrafast) - 旗艦迴圈；Flights ~7s／~$0.0039。X：[@gregpr07](https://x.com/gregpr07/status/2100411066966749359)。
...
- [jev-browser-pilot](https://github.com/aidil2105/jev-browser-pilot) - 函式庫型 pilot；星數低、結構強。

**暫不收錄：** 無新量測／無 X 聲量的 Ultrafast 薄包裝（含 mobile，等有討論再看）。

## 程式碼代理

- [fast-jev-compaction](https://github.com/tamaratran/fast-jev-compaction) - Claude Code compaction；高星／趨勢強，無 launch 帖仍 keep。
- [foreman](https://github.com/thruwire/foreman) - 軟體工廠監督。X：[@JoshARosen](https://x.com/JoshARosen/status/2100573432089866717)。
- [jev-review（devagrawal09）](https://github.com/devagrawal09/jev-review) - 分階段 review；見 roundup [@0xLogicrw](https://x.com/0xLogicrw/status/2100478725393686556)。
- [jev-review（NiazMorshed2007）](https://github.com/NiazMorshed2007/jev-review) - MCP-first 時再優先。
- [pi-jev](https://github.com/y0usaf/pi-jev) - Pi 決策層。
- [pi-warden](https://github.com/DevMortimer/pi-warden) - Pi 護欄。

## 路由器

- [jev-router](https://github.com/gargpratyush/jev-router) - 最常被引用的 router；X 弱於瀏覽器 demo，仍因實用 keep。
- [hono-jev-router](https://github.com/yusukebe/hono-jev-router) - Hono 語意路由。

## 資料與函式庫

- [pg-jev](https://github.com/realZachi/pg-jev) - Postgres 擴充；X 弱、niche 實用強故 keep。

## 值得研究的 Demo

- [jev-trader](https://github.com/jarrodwatts/jev-trader) - Monad block 交易決策。X：[@jarrodwatts](https://x.com/jarrodwatts/status/2100356151468585346)。
- [typesafe-mario](https://github.com/fhshaik/typesafe-mario) - 結構化狀態玩 Mario。

## Jev-like 與相關模型

可不呼叫 TypeSafe；收錄看**討論熱度與研究價值**。

- [SemIf](https://github.com/TheoLeeCJ/SemIf) - 開源「語意 if」。X 稀疏（如 [@hhkkmon](https://x.com/hhkkmon/status/2100443314957038010)）；標 inspired-by。
- [jevlike](https://github.com/vinnylarouge/jevlike) - 小模型一次通過選項機率；X 稀疏 + HN ~161 pts。
- [nimble](https://github.com/bespokelabsai/nimble) - 開源「Jev 配方」（LoRA＋constrained serving）。非官方。X：[@madiator](https://x.com/madiator/status/2100990591215783946)；亦上 HN。
- [openjev-sglang](https://github.com/ekzhang/openjev-sglang) - Prefill-only、相容 Jev 形狀的 API（SGLang）。非官方。HN 有討論。
- [open-jev](https://github.com/daseinlabs/open-jev) - 本機 Gemma／MLX option scoring。非官方。HN。
- [mini-jev](https://github.com/r-ms/mini-jev) - Qwen3 letter-logits 小實驗。非官方。HN。
- [jevmlx](https://github.com/bnsd55/jevmlx) - Apple Silicon MLX 平行決策。非官方。X：[@beni_il_](https://x.com/beni_il_/status/2100617387116568956)；HN。
- [Qwen-2.5-1B-RLCD](https://huggingface.co/harshatheg/Qwen-2.5-1B-RLCD) - 平行 constrained／多字段（HF 模型）。非官方。強 X：[@harshagundal](https://x.com/harshagundal/status/2100044305536889015)。
- [LitJev](https://github.com/zhengxuyu/LitJev) - 任意 LLM 包成類 Jev `/v1/systemone`。非官方。X：[@yuzxfred](https://x.com/yuzxfred/status/2100652136878981337)。
